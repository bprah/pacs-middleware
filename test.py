...
DATAFILES_URL = f"{BASE_URL}/files"
...
def login_and_get_token():
    ...
    # --- handle TOTP setup or use existing secret ---
    ...
    if r.status_code == 400 and "TOTP code required" in r.json().get("detail", ""):
        secret = (
            td.get("totp_secret") or input("Enter your Base32 TOTP secret: ").strip()
        )
        tok = pyotp.TOTP(secret).now()
        r2 = requests.post(
            LOGIN_URL,
            json={"email": ADMIN_EMAIL, "password": ADMIN_PASSWORD, "totp_code": tok},
        )
        r2.raise_for_status()
        token = r2.json()["access_token"]
...
def test_create_patient(token):
    ...
    files = {
        "informed_consent_doc": open("consent.pdf", "rb"),
        "related_reports_doc": open("related.pdf", "rb"),
    }
    r = requests.post(
        PATIENTS_URL,
...
def test_get_by_id(token, pid):
    r = requests.get(
        f"{PATIENTS_URL}/{pid}", headers={"Authorization": f"Bearer {token}"}
    )
    assert r.status_code == 200, f"Expected 200, got {r.status_code}"
...
def test_verify_in_list(token, pid):
    r = requests.get(PATIENTS_URL, headers={"Authorization": f"Bearer {token}"})
    assert any(p["id"] == pid for p in r.json()), f"Patient {pid} not found"
    print(f"✔ Patient {pid} found in list")
...
# ── DataFiles Tests ────────────────────────────────────────
def test_files_list_no_auth():
    r = requests.get(DATAFILES_URL)
    assert r.status_code == 401, f"Expected 401, got {r.status_code}"
    print("✔ Unauthorized /files list passed")


def test_files_list_with_auth(token):
    r = requests.get(DATAFILES_URL, headers={"Authorization": f"Bearer {token}"})
    assert r.status_code == 200, f"Expected 200, got {r.status_code}"
    initial = r.json()
    print(f"✔ Authorized /files list returned {len(initial)} record(s)")
    return initial


def test_files_create_invalid_dicom(token):
    # send a plain-text file as "dicom"
    fake = ("notdicom.txt", b"this is not dicom")
    files = {"upload": fake}
    data = {
        "data_name": "Bad DCM",
        "project_id": 1,
    ...
    print("✔ Invalid DICOM upload correctly rejected")


def test_files_create_dicom(token):
    # Use the real 1.dcm fixture on disk
    dicom_path = "1.dcm"
    if not os.path.exists(dicom_path):
        pytest.skip("1.dcm not found")
    with open(dicom_path, "rb") as f:
        dicom_bytes = f.read()

    files = {"upload": ("1.dcm", dicom_bytes)}
    data = {
        "data_name": "Test DICOM (real file)",
        "project_id": 1,
        "patient_id": 1,
        "modality": "CT",
    ...
    assert r.status_code == 201, f"DICOM upload failed: {r.status_code} {r.text}"
    rec = r.json()
    assert rec.get("orthanc_id"), "orthanc_id missing"
    print(f"✔ DICOM uploaded → orthanc_id={rec['orthanc_id']}")
    return rec["id"]

def test_files_create_pdf(token):
    # a dummy PDF
    pdf_path = "dummy.pdf"
    if not os.path.exists(pdf_path):
        open(pdf_path, "wb").write(b"%PDF-1.4\n%Dummy\n")
    files = {"upload": open(pdf_path, "rb")}
    data = {
        "data_name": "Test PDF",
        "project_id": 1,
        "patient_id": 1,
    ...
    files["upload"].close()
    assert r.status_code == 201, f"PDF upload failed: {r.status_code}"
    rec = r.json()
    assert rec.get("storage_path"), "storage_path missing"
    print(f"✔ PDF uploaded → storage_path={rec['storage_path']}")
    return rec["id"]


def test_files_list_contains(token, before, did_ids):
    r = requests.get(DATAFILES_URL, headers={"Authorization": f"Bearer {token}"})
    arr = r.json()
    new_ids = [f["id"] for f in arr if f["id"] not in {p["id"] for p in before}]
    assert set(new_ids) >= set(did_ids), f"New uploads {did_ids} missing in {new_ids}"
    print(f"✔ /files list now contains uploads {did_ids}")
...
def test_register_and_pending_flow(token):
    ...
    r = requests.post(
        REGISTER_URL, headers={"Authorization": f"Bearer {token}"}, data=data
    )
    ...
    assert r2.status_code == 200 and any(p["id"] == pid for p in r2.json()), (
        "Pending list missing registration"
    )
    print("✔ Pending list contains new registration")
    ...
    assert r3.status_code == 200, f"Approve failed: {r3.status_code}"
    print(f"✔ Approved pending user → new user id={r3.json()['id']}")
...
def main():
    ...
    test_projects_update(token, proj_id)
    test_projects_update_nonexistent(token)

    # files
    before = test_files_list_with_auth(token)
    test_files_list_no_auth()
    test_files_create_invalid_dicom(token)
    dicom_id = test_files_create_dicom(token)
    pdf_id = test_files_create_pdf(token)
    test_files_list_contains(token, before, [dicom_id, pdf_id])
...
def main():
    ...
    def run(test_fn, *args):
        """Helper to run a test and record failures without stopping."""
    ...
    # patients
    run(test_list_no_auth)
    run(test_list_with_auth, token)
    pid = run(test_create_patient, token)
    run(test_get_by_id, token, pid)
    run(test_update_patient, token, pid)
    run(test_verify_in_list, token, pid)

    # projects
    run(test_projects_list_no_auth)
    run(test_projects_list_with_auth, token)
    proj = run(test_projects_create_valid, token)
    if proj:
        proj_id = proj[0]
        run(test_projects_update, token, proj_id)
    run(test_projects_update_nonexistent, token)

    # files
    before = run(test_files_list_with_auth, token)
    run(test_files_list_no_auth)
    run(test_files_create_invalid_dicom, token)
    dicom_id = run(test_files_create_dicom, token)
    pdf_id   = run(test_files_create_pdf, token)
    if before is not None and dicom_id and pdf_id:
        run(test_files_list_contains, token, before, [dicom_id, pdf_id])

    # admin/register
    run(test_register_and_pending_flow, token)

    # me
    run(test_me_endpoint, token)
